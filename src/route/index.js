...
Product.add(
  ...
  `Intel Xeon E-2226G (3.4 - 4.7 ГГц) / RAM 16 ГБ / SSD 512 ГБ / nVidia Quadro P620, 2 ГБ / DVD+/-RW / LAN / без ОС`,
  [{ id: 1, text: 'Готовий до відправки' }],
  40000,
  10,
)
...
class Promoсode {
  ...
  static add = (name, factor) => {
    const newPromoCode = new Promoсode(name, factor)
    Promoсode.#list.push(newPromoCode)
    return newPromoCode
  }
...
Promoсode.add('SUMMER2023', 0.9)
Promoсode.add('DISCOUNT50', 0.5)
Promoсode.add('SALE25', 0.75)
...
router.post('/purchase-submit', function (req, res) {
  ...
  if (promocode) {
    promocode = Promoсode.getByName(promocode)

    if (promocode) {
      totalPrice = Promoсode.calc(promocode, totalPrice)
    }
  }
